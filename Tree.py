...
def chooseBestFeatureToSplit(dataSet,features):
    ...
    for i in range(numFeatures):
        ...
        # 打印每个特征的信息增益
        print("第%d个特征%s的增益为%.3f" % (i,features[i] ,infoGain))
        featuresShannonDic.update({features[i]:infoGain})
        # 计算信息增益
    ...
    return bestFeature,featuresShannonDic
...
if __name__ == '__main__':
    ...
    bestFeature,featuresShannonDic=chooseBestFeatureToSplit(dataSet,features)
    # print("最优特征索引值:" + str(bestFeature))

    dataSet_order=sorted(featuresShannonDic.items(), key=lambda x: x[1],reverse=True)

    print('-----------------------------------------------------')
    i=0
    for fe in enumerate(dataSet_order):
        i+=1
        # print("NO%d  %s的增益为%.3f" % (i,fe[1][0] , fe[1][1]))
        print("The information gain value of the %dth  %s is : %.3f" % (i,fe[1][0] , fe[1][1]))
        # print(type(fe))

    # print(type(dataSet_order))
    # print(sorted(featuresShannonDic.items(), lambda x, y: cmp(x[1], y[1]), reverse=True))
